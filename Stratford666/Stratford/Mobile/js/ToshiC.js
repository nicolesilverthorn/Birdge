$(document).ready(function()
{
	...
	var _cameraClass =
	{
		...
		update : function(_player)
		{
			...
			if(_player._spriteAnimation.x >= _camera.rightInnerBoundary())
			{
				...
				_godzilla._spriteAnimation.x += 90;
				_walkLeft == false;
				_walkRight == true;
				_carlos == true;
			}
	...
	spriteAtlas.src = "images/ToshiSheet2.png";
	spriteAtlas.addEventListener("load", assetLoaded, false);
	_assetsToLoad.push(spriteAtlas);
	
	var backgroundImage = new Image();
	backgroundImage.src = "images/AnimationBackgroundT.png";
	...
	//Billboards
	var _bomb5 = Object.create(GameObjectClass);
	gameObjects.push(_bomb5);

	//Billboards
	var _bomb6 = Object.create(GameObjectClass);
	gameObjects.push(_bomb6);
	...
	//load Billboards sprite
	var _bombSprite5 = Object.create(_spriteAnimationClass);
	_bomb5._spriteAnimation = _bombSprite5;
	_bomb5._spriteAnimation.image = spriteAtlas;
	
	//load Billboards 2 sprite
	var _bombSprite6 = Object.create(_spriteAnimationClass);
	_bomb6._spriteAnimation = _bombSprite6;
	_bomb6._spriteAnimation.image = spriteAtlas;
	...
	//update anim code here
		...
		if(_walkRight)
		{
			...
			_player._spriteAnimation.init(spriteAtlas, 94, 94, 0, 60, 148);  //walk right
	...
	function gameLoaded()
	{
		...
		//Toshi
			//Billboards 1
		_bomb5._spriteAnimation.init(spriteAtlas, 94, 94, 74, 11, 128); //Helena idle frame
		_bomb5._spriteAnimation.x = _player_START_X + 650;
		_bomb5._spriteAnimation.y  = _player_START_Y - 120;
		
		//Billboards 2
		_bomb6._spriteAnimation.init(spriteAtlas, 94, 94, 74, 11, 128); //Helena idle frame
		_bomb6._spriteAnimation.x = _player_START_X + 150;
		_bomb6._spriteAnimation.y  = _player_START_Y - 120;
	
		_godzilla._spriteAnimation.init(spriteAtlas, 94, 94, 60, 10, 128); //Toshi Left idle frame
		_godzilla._spriteAnimation.x = _player_START_X + 170 ;
		_godzilla._spriteAnimation.y  = _player_START_Y - 70;
		
	
		//bombz
		_bomb._spriteAnimation.init(spriteAtlas, 94, 94, 70, 4, 128); //Helena idle frame
		_bomb._spriteAnimation.x = _player_START_X + 350;
		_bomb._spriteAnimation.y  = _player_START_Y - 90;
		
		
		//fireworks
		_bomb2._spriteAnimation.init(spriteAtlas, 94, 94, 70, 4, 128); //Helena idle frame
		_bomb2._spriteAnimation.x = _player_START_X + 252;
		_bomb2._spriteAnimation.y  = _player_START_Y - 46;
		
		
		_bomb3._spriteAnimation.init(spriteAtlas, 94, 94, 70, 4, 128); //Helena idle frame
		_bomb3._spriteAnimation.x = _player_START_X + 650;
		_bomb3._spriteAnimation.y  = _player_START_Y - 90;
		
		
		_bomb4._spriteAnimation.init(spriteAtlas, 94, 94, 70, 4, 128); //Helena idle frame
		...
		_player._spriteAnimation.y  = _player_START_Y - 50;
